...
-   `urls` **([Array][13]&lt;[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][20] to learn more about the actual url format.
...
-   `customParameters` **[Array][13]&lt;[call.CustomParameter][26]>** The custom parameters set for the call.
...
-   `options` **[Object][7]?** 
    ...
    -   `options.customParameters` **[Array][13]&lt;[call.CustomParameter][26]>?** Custom SIP header parameters for the SIP backend.
...
-   `customParameters` **[Array][13]&lt;[call.CustomParameter][26]>** The custom parameters to set.
...
The same behaviour as the `changeSpeaker` API can be implemented by
   ...
   by unrendering the track with [media.removeTracks][58], then
...
[65]: conversation.Message.addPart
