...
*   `customParameters` **[Array][14]<[call.CustomParameter][32]>** The locally set Custom Parameters for the call.
...
*   `urls` **([Array][14]<[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][35] to learn more about the actual url format.
...
*   `tracks` **[Array][14]<[call.TrackObject][37]>** A list of Track objects that are contained in this Media object.
...
*   `camera` **[Array][14]<[call.DeviceInfo][38]>** A list of camera device information.
*   `microphone` **[Array][14]<[call.DeviceInfo][38]>** A list of microphone device information.
*   `speaker` **[Array][14]<[call.DeviceInfo][38]>** A list of speaker device information.
...
[call.getAll][25] APIs. These are the parameters that will be sent to the remote
...
*   `options` **[Object][7]?** 
    ...
    *   `options.customParameters` **[Array][14]<[call.CustomParameter][32]>?** Custom SIP header parameters for the SIP backend.
...
*   `callId` **[string][8]** The ID of the call.
*   `customParameters` **[Array][14]<[call.CustomParameter][32]>** The custom parameters to set.
...
Returns **[Array][14]<[call.CallObject][58]>** Call objects.
...
The SDK will emit a [call:trackEnded][59]
...
The data retrieved is a [RTCStatsReport][62]
...
This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][67].
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][69] API.
...
*   `params` **[Object][7]** 
    ...
    *   `params.customParameters` **[Array][14]<[call.CustomParameter][32]>** The custom parameters received.
...
The data can be retrieved from the [call.history.getCache][83] API. This allows an
application to persist the information across SDK instances when the
backend environment does not support the CallHistory feature.
...
Returns **[Array][14]<[conversation.Conversation][86]>** An array of conversation objects.
...
[MediaConstraint(s)][103] that was provided.

#### Parameters

*   `params` **[Object][7]** 

    *   `params.error` **[Error][21]** The Basic error object.

### media:muted

The specified Tracks have been muted.

A Track can be muted using the [media.muteTracks][104] API.
...
A Track can be unmuted using the [media.unmuteTracks][105]
API.
...
[media:sourceMuted][107] event), and will
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][108]
...
Returns **[Promise][109]** When successful,  the information of the registration.
...
Returns **[Promise][109]** When successful, the promise will resolve with undefined.
Promise will reject with error object otherwise.
...
This is a wrapper on top of the browser's [fetch API][121]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][121]
...
Returns **[Promise][109]<[Response][122]>** A promise for a [Response][123] object.
...
initialization of the SDK using the [config.call.sdpHandlers][124] configuration
parameter. They can also be set after the SDK's creation by using the
[call.setSdpHandlers][125] function.
...
removal handler in combination with the [call.getAvailableCodecs][80] function in order to build logic to determine the best codecs to use
...
using the [services.getSubscriptions][126] API. These values can be used
...
This is the configuration object that needs to be passed as the value for the [ServiceDescriptor.params][127] property.
...
For push notifications on link, please see [notifications.registerPush][128]
...
*   `services` **[Array][14]<([string][8] | [services.ServiceDescriptor][129])>** A list of service configurations.
*   `options` **[Object][7]?** The options object for non-credential options.
...
[services.getSubscriptions][126] API. The `subscribed` values are the
...
[services.getSubscriptions][126] API.
...
[sip.subscribe API][130] for more information about solicited events.
...
from a subscription using the [sip.update][131] API at any time.

The SDK will emit a [sip:subscriptionChange][132]
event when the operations completes. The [sip.getDetails][133] API can be used
to retrieve the current information about a subscription.

The SDK will emit a [sip:eventsChange][134] event when
...
*   `customParameters` **[Array][14]<[call.CustomParameter][32]>?** Custom SIP header parameters for the SIP backend.
...
Subscription details will no longer be available using the [sip.getDetails][133]
...
This event can be emitted when a new SIP subscription is created ([sip.subscribe][130]
API), an existing subscription is updated ([sip.update][131] API), or has been
deleted ([sip.unsubscribe][135] API). The `change` parameter on the event indicates
...
The [sip.getDetails][133] API can be used to retrieve the current information about
a subscription.
...
Compared to [user.fetch][138] API, this API retrieves additional user related information.
...
Returns **[Array][14]<[user.User][140]>** An array of all the User objects.
...
*   `params` **[Object][7]** 
    ...
    *   `params.results` **[Array][14]<[user.User][140]>** The Users' information returned by the
        operation.
...
A [voicemail:change][143] event is
...
[34]: https://developer.mozilla.org/en-US/docs/Web/API/RTCIceServer#properties

[35]: https://developer.mozilla.org/en-US/docs/Web/API/RTCIceServer/urls

[36]: #callsdphandlerinfo
...
[41]: #callsetcustomparameters

[42]: #callsendcustomparameters

[43]: #calleventcallcustomparameters
...
[47]: #calleventcalloperation

[48]: #calleventcallstart

[49]: #calleventcallreceive

[50]: #callsip_uri

[51]: #calltel_uri

[52]: #callmediaconstraint

[53]: #calldscpcontrols

[54]: #calleventcallstatechange

[55]: #calleventcallnewtrack

[56]: #callunhold

[57]: #callcustomparameter

[58]: #callcallobject

[59]: #calleventcalltrackended

[60]: #calladdmedia

[61]: #callremovemedia

[62]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[63]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[64]: #calleventcallstatsreceived

[65]: #calleventcalltrackreplaced

[66]: #calleventcallavailablecodecs

[67]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities

[68]: #callsdphandlerfunction

[69]: #callrestartmedia

[70]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[71]: #calleventcallmediaconnectionchange

[72]: #calleventcallmediarestart

[73]: #mediarendertracks

[74]: #callreplacetrack

[75]: #mediaremovetracks

[76]: #mediagettrackbyid

[77]: #calleventcallremovedmedia

[78]: #callgetstats

[79]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[80]: #callgetavailablecodecs

[81]: #callhistoryget

[82]: callHistory.setCache

[83]: callHistory.getCache

[84]: #connectiongetsocketstate

[85]: #connectionwsconnectionobject

[86]: #conversationconversation

[87]: #conversationconversationcreatemessage

[88]: #conversationmessage

[89]: conversation.Message.addPart

[90]: #conversationmessagesend

[91]: #conversationconversationgetmessages

[92]: #conversationconversationgetmessage

[93]: conversation.chatTypes
...
[103]: #callmediaconstraint

[104]: #mediamutetracks

[105]: #mediaunmutetracks

[106]: #mediaeventmediasourceunmuted

[107]: #mediaeventmediasourcemuted

[108]: api.notifications.process
...
[121]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[122]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5

[123]: https://developer.mozilla.org/en-US/docs/Web/API/Response

[124]: #configconfigcall

[125]: #callsetsdphandlers

[126]: #servicesgetsubscriptions

[127]: #servicesservicedescriptor

[128]: notifications.registerPush
...
[132]: #sipeventsipsubscriptionchange

[133]: #sipgetdetails

[134]: #sipeventsipeventschange
...
[143]: #voicemaileventvoicemailchange
