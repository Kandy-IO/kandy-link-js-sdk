...
*   `customParameters` **[Array][14]<[call.CustomParameter][31]>** The locally set Custom Parameters for the call.
...
*   `urls` **([Array][14]<[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][33] to learn more about the actual url format.
...
*   `tracks` **[Array][14]<[call.TrackObject][35]>** A list of Track objects that are contained in this Media object.
...
*   `camera` **[Array][14]<[call.DeviceInfo][36]>** A list of camera device information.
*   `microphone` **[Array][14]<[call.DeviceInfo][36]>** A list of microphone device information.
*   `speaker` **[Array][14]<[call.DeviceInfo][36]>** A list of speaker device information.
...
These parameters can be specified with the [call.make][37] and [call.answer][38] APIs.
They can also be set after a Call is established using the [call.setCustomParameters][39] API, and sent using the [call.sendCustomParameters][40] API.
...
When these custom headers are received, the SDK will emit a [call:customParameters][41] event
which will contain the custom parameters that were received.

A Call's custom parameters are stored on the Call's [CallObject][42],
...
part of the CallObject and are only provided via the [call:customParameters][41] event.
...
Starts an outgoing call to a [SIP_URI][43] or a
[TEL_URI][44].
...
[call:operation][45] event.

The SDK will emit a [call:start][46] event
locally when the operation completes. When the remote participant
receives the call, a [call:receive][47]
...
*   `destination` **([call.SIP_URI][48] | [call.TEL_URI][49])** The desired destination.
*   `media` **[Object][7]** The media options the call should be initialized with.
    ...
    *   `media.audioOptions` **[Object][7]?** Options for configuring the call's audio.
        ...
        *   `media.audioOptions.deviceId` **[call.MediaConstraint][50]?** ID of the microphone to receive audio from.
    ...
    *   `media.videoOptions` **[Object][7]?** Options for configuring the call's video.
        ...
        *   `media.videoOptions.deviceId` **[call.MediaConstraint][50]?** ID of the camera to receive video from.
        *   `media.videoOptions.height` **[call.MediaConstraint][50]?** The height of the video.
        *   `media.videoOptions.width` **[call.MediaConstraint][50]?** The width of the video.
        *   `media.videoOptions.frameRate` **[call.MediaConstraint][50]?** The frame rate of the video.
    ...
    *   `media.screenOptions` **[Object][7]?** Options for configuring the call's screenShare.
        ...
        *   `media.screenOptions.height` **[call.MediaConstraint][50]?** The height of the screenShare.
        *   `media.screenOptions.width` **[call.MediaConstraint][50]?** The width of the screenShare.
        *   `media.screenOptions.frameRate` **[call.MediaConstraint][50]?** The frame rate of the screenShare.
*   `options` **[Object][7]?** 
    ...
    *   `options.customParameters` **[Array][14]<[call.CustomParameter][31]>?** Custom SIP header parameters for the SIP backend.
    *   `options.dscpControls` **[call.DSCPControls][51]?** Options for configuring DSCP markings on the media traffic
...
[call:newTrack][53] event for more
...
call can be taken off hold with the [call.unhold][54] API.
...
Set the [Custom Parameters][55] of a Call, to be provided to the remote endpoint.
...
A Call's custom parameters are a property of the Call's [CallObject][42],
which can be retrieved using the [call.getById][25] or
[call.getAll][24] APIs.

The custom parameters set on a call can be sent directly with the [call.sendCustomParameters][40] API.
...
*   `callId` **[string][8]** The ID of the call.
*   `customParameters` **[Array][14]<[call.CustomParameter][31]>** The custom parameters to set.
...
To change or remove the custom parameters on a call, use the [call.setCustomParameters][39] API.
...
Returns **[Array][14]<[call.CallObject][56]>** Call objects.
...
The SDK will emit a [call:newTrack][53] event
...
*   `videoOptions` **[Object][7]?** Options for configuring the call's video.
    ...
    *   `videoOptions.deviceId` **[call.MediaConstraint][50]?** ID of the camera to receive video from.
    *   `videoOptions.height` **[call.MediaConstraint][50]?** The height of the video.
    *   `videoOptions.width` **[call.MediaConstraint][50]?** The width of the video.
    *   `videoOptions.frameRate` **[call.MediaConstraint][50]?** The frame rate of the video.
*   `options` **[Object][7]?** 
...
*   `callId` **[string][8]** ID of the call being acted on.
*   `destination` **([call.SIP_URI][48] | [call.TEL_URI][49])** The destination to forward the call to.
...
The progression of the operation will be tracked via the
[call:operation][45] event. The remote
...
*   `callId` **[string][8]** ID of the call being acted on.
*   `destination` **([call.SIP_URI][48] | [call.TEL_URI][49])** The destination to transfer the call to.
...
The progress of the operation will be tracked via the
[call:operation][45] event. Both remote
...
problem. Using the [call.restartMedia][67] API will attempt to reconnect the media. See
the [call.restartMedia][67] API description for more information.

These states are direct reflections of the possible
[RTCPeerConnection.iceConnectionState][68] values.

The Call's media connection state is a property of the [CallObject][42],
which can be retrieved using the [call.getById][25] or
[call.getAll][24] APIs.
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][67] API.
...
using the general-purpose [call.replaceTrack][72] API. This API can
...
by unrendering the track with [media.removeTracks][73], then
re-rendering it with a new speaker with [media.renderTracks][71].
...
its media removed, see [call:removedMedia][75]
...
See the [call.getStats][76] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][77]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][23]?** An error object, if the operation was not successful.
...
A Track is replaced by the local user using the [call.replaceTrack][72]
API.

This event is similar to the [call:newTrack][53]
...
*   `params` **[Object][7]** 
    ...
    *   `params.oldTrack` **[call.TrackObject][35]?** State of the replaced track.
    *   `params.error` **[api.BasicError][23]?** An error object, if the operation was not successful.
...
[CustomParameter][55] for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.customParameters` **[Array][14]<[call.CustomParameter][31]>** The custom parameters received.

### call:availableCodecs

The list of available and supported codecs by the browser have been retrieved.

This event is emitted as a result of the [call.getAvailableCodecs][78] API. Please refer to the API for more
information.
...
This event is emitted as a result of the [call.restartMedia][67] API being called.
See the description for [call.restartMedia][67] for information about its
usage.
...
[CallHistory.get][79] can then be used to get
...
[call.history.setCache][80] API. This allows an
...
The data can be retrieved from the [call.history.getCache][81] API. This allows an
application to persist the information across SDK instances when the
backend environment does not support the CallHistory feature.
...
Call history state has been updated. See [CallHistory.get][79] to retrieve new state.
...
Returns **[conversation.Conversation][84]** A Conversation object matching the passed destination, otherwise undefined is returned.

### getAll

Returns all conversations currently tracked by the SDK

Returns **[Array][14]<[conversation.Conversation][84]>** An array of conversation objects.
...
A message object can be obtained through the [Conversation.createMessage][85] API on an existing conversation.
...
the message can then be sent using the [Message.send][88] function.
...
obtained through the [Conversation.getMessages][89] or [Conversation.getMessage][90] API on an existing conversation.
...
*   `type` **[string][8]** The type of message that was sent. See [conversation.chatTypes][91] for valid types.
...
Returns **[call.TrackObject][35]** A Track object.
...
[MediaConstraint(s)][101] that was provided.
...
A Track can be muted using the [media.muteTracks][102] API.
...
A Track can be unmuted using the [media.unmuteTracks][103]
API.
...
[media:sourceMuted][105] event), and will
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][106]
...
Returns **[Promise][107]** When successful,  the information of the registration.
...
Returns **[Promise][107]** When successful, the promise will resolve with undefined.
Promise will reject with error object otherwise.
...
available either by using [presence.fetch][108] or by subscribing for
updates about other Users, using [presence.subscribe][109].

Available presence information can be retrieved using [presence.get][110] or
[presence.getAll][111].
...
An instance of this type can be obtained by invoking the [presence.get][110] function.
...
The status is set to [open][112] as soon as a user subscribes for the presence service.
...
*   `status` **[string][8]** The current status the user has set for themselves. For supported values see [presence.statuses][112].
*   `activity` **[string][8]** The current activity of the user.
    For supported values see [presence.activities][113].
...
See [presence.statuses][112] and [presence.activities][113] for valid
values.

The SDK will emit a
[presence:selfChange][114] event
when the operation completes. The updated presence information is
available and can be retrieved with [presence.getSelf][115].

Other users subscribed for this user's presence will receive a
[presence:change][116] event.
...
This information is set using the [presence.update][117] API.
...
[presence.get][110] or [presence.getAll][111] APIs.
...
This event is generated as a result of [presence.fetch][108] or [presence.update][117] operations.
...
The changed information can be retrieved using the [presence.get][110]
API.
...
The changed information can be retrieved using the [presence.getSelf][115]
API.
...
*   `channel` **[proxy.Channel][118]** See the `Channel` module for information.
...
This is a wrapper on top of the browser's [fetch API][119]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][119]
...
Returns **[Promise][107]<[Response][120]>** A promise for a [Response][121] object.
...
removal handler in combination with the [call.getAvailableCodecs][78] function in order to build logic to determine the best codecs to use
...
*   `customParameters` **[Array][14]<[call.CustomParameter][31]>?** Custom SIP header parameters for the SIP backend.
...
[39]: #callsetcustomparameters

[40]: #callsendcustomparameters

[41]: #calleventcallcustomparameters
...
[45]: #calleventcalloperation

[46]: #calleventcallstart

[47]: #calleventcallreceive

[48]: #callsip_uri

[49]: #calltel_uri

[50]: #callmediaconstraint

[51]: #calldscpcontrols

[52]: #calleventcallstatechange

[53]: #calleventcallnewtrack

[54]: #callunhold

[55]: #callcustomparameter
...
[68]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[69]: #calleventcallmediaconnectionchange

[70]: #calleventcallmediarestart

[71]: #mediarendertracks

[72]: #callreplacetrack

[73]: #mediaremovetracks

[74]: #mediagettrackbyid

[75]: #calleventcallremovedmedia

[76]: #callgetstats

[77]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[78]: #callgetavailablecodecs

[79]: #callhistoryget

[80]: callHistory.setCache

[81]: callHistory.getCache

[82]: #connectiongetsocketstate

[83]: #connectionwsconnectionobject

[84]: #conversationconversation

[85]: #conversationconversationcreatemessage

[86]: #conversationmessage

[87]: conversation.Message.addPart

[88]: #conversationmessagesend

[89]: #conversationconversationgetmessages

[90]: #conversationconversationgetmessage

[91]: conversation.chatTypes
...
[101]: #callmediaconstraint

[102]: #mediamutetracks

[103]: #mediaunmutetracks

[104]: #mediaeventmediasourceunmuted

[105]: #mediaeventmediasourcemuted

[106]: api.notifications.process

[107]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[108]: #presencefetch

[109]: #presencesubscribe

[110]: #presenceget

[111]: #presencegetall

[112]: #presencestatuses

[113]: #presenceactivities

[114]: #presenceeventpresenceselfchange

[115]: #presencegetself

[116]: #presenceeventpresencechange

[117]: #presenceupdate

[118]: #proxychannel

[119]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[120]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5

[121]: https://developer.mozilla.org/en-US/docs/Web/API/Response
