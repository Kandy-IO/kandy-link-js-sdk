...
-   `authentication` **[Object][4]** Authentication configs.
    -   `authentication.subscription` **[Object][4]** 
        ...
        -   `authentication.subscription.service` **[Array][7]?** Services to subscribe to for notifications.
...
-   `call` **[Object][4]** The call configuration object.
    ...
    -   `call.iceServers` **[Array][7]&lt;[call.IceServer][8]>?** The list of ICE servers to be used for calls.
    -   `call.iceCollectionDelay` **[number][6]** Time, in milliseconds, to delay in between
    ...
    -   `call.maxIceTimeout` **[number][6]** Maximum time, in milliseconds, to wait for ICE
    ...
    -   `call.iceCollectionCheck` **[Function][9]?** Function to check whether collected candidates
    ...
    -   `call.sdpHandlers` **[Array][7]&lt;[call.SdpHandlerFunction][11]>?** List of SDP handler functions to modify SDP. Advanced usage.
...
-   `pingInterval` **[Number][6]** Time in between websocket ping attempts (milliseconds). (optional, default `30000`)
-   `reconnectLimit` **[Number][6]** Number of failed reconnect attempts before reporting an error. Can be set to 0 to not limit reconnection attempts. (optional, default `5`)
-   `reconnectDelay` **[Number][6]** Base time between websocket reconnect attempts (milliseconds). (optional, default `5000`)
-   `reconnectTimeMultiplier` **[Number][6]** Reconnect delay multiplier for subsequent attempts. The reconnect delay time will be multiplied by this factor after each failed reconnect attempt to increase the delay between attempts. (optional, default `1`)
-   `reconnectTimeLimit` **[Number][6]** Maximum time delay between reconnect attempts (milliseconds). Used in conjunction with the reconnect time multiplier to prevent overly long delays between reconnection attempts. (optional, default `640000`)
...
-   `maxMissedPings` **[Number][6]** Maximum pings sent (without receiving a response) before reporting an error. (optional, default `3`)
...
-   `notifications` **[Object][4]** The notifications configuration object.
    ...
    -   `notifications.incomingCallNotificationMode` **[string][5]** Communication channel mode used for incoming call notifications. Supported values are 'any-channel' or 'push-channel-only'. (optional, default `'any-channel'`)
...
The format of logs can also be customized by providing a
   [LogHandler][12]. This function will receive a
   [LogEntry][13] which it can handle as it sees fit. By
   default, the SDK will log information to the console.

**Parameters**

-   `logs` **[Object][4]** Logs configs.
    -   `logs.logLevel` **[string][5]** Log level to be set. See [logger.levels][14]. (optional, default `'debug'`)
    -   `logs.handler` **[logger.LogHandler][15]?** The function to receive log entries from the
    ...
    -   `logs.logActions` **[Object][4]?** Options specifically for action logs when
           logLevel is at DEBUG+ levels. Set this to false to not output action logs.
        -   `logs.logActions.handler` **[logger.LogHandler][15]?** The function to receive action
...
-   `newConfigValues` **[Object][4]** Key-value pairs that will be placed into the store. See [config][16] for details on what key-value pairs are available for use.
...
-   `listener` **[Function][9]** The listener for the event type. The parameters of the listener depend on the event type.
...
-   `listener` **[Function][9]** The listener to remove.
...
-   `listener` **[Function][9]** The event listener to add. The parameters are (type, ...args), where args depend on the event type.
...
-   `listener` **[Function][9]** The event listener to remove.
...
-   `options` **[Object][4]?** The options object for non-credential options.
...
-   `connection` **[Object][4]** 
    -   `connection.services` **[Array][7]** Services to subscribe to for notifications.
...
Returns **[Array][7]** A list of subscribed-to services.
...
-   `camera` **[Array][7]&lt;[call.DeviceInfo][18]>** A list of camera device information.
-   `microphone` **[Array][7]&lt;[call.DeviceInfo][18]>** A list of microphone device information.
-   `speaker` **[Array][7]&lt;[call.DeviceInfo][18]>** A list of speaker device information.
...
These headers can be specified with the [call.make][19] and [call.answer][20] APIs.
They can also be set on a call using the [call.setCustomParameters][21], and sent using the [call.sendCustomParameters][22] API.
...
Custom headers may be received anytime throughout the duration a call. A remote endpoint may send custom headers when starting a call,
 ...
 When these custom headers are received, the SDK will emit a [call:customParameters][23] event
...
-   `containers` **[Array][7]&lt;[string][5]>** The list of CSS selectors that were used to render this Track.
...
-   `tracks` **[Array][7]&lt;[call.TrackObject][27]>** A list of Track objects that are contained in this Media object.
...
-   `urls` **([Array][7]&lt;[string][5]> | [string][5])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][29] to learn more about the actual url format.
...
Can be retrieved using the [call.getAll][26] or [call.getById][25] APIs.
...
-   `state` **[string][5]** The current state of the call. See [call.states][30] for possible states.
...
-   `localTracks` **[Array][7]&lt;[string][5]>** A list of Track IDs that the call is sending to the remote participant.
-   `remoteTracks` **[Array][7]&lt;[string][5]>** A list of Track IDs that the call is receiving from the remote participant.
...
-   `customParameters` **[Array][7]&lt;[call.CustomParameter][32]>** The custom parameters set for the call.
...
Starts an outgoing call to a [SIP_URI][33] or a
   [TEL_URI][34].
...
The progress of the operation will be tracked via the
   [call:operation][35] event.

The SDK will emit a [call:start][36] event
   locally when the operation completes. When the remote participant
   receives the call, a [call:receive][37]
...
-   `destination` **([call.SIP_URI][38] \| [call.TEL_URI][39])** The desired destination.
-   `media` **[Object][4]** The media options the call should be initialized with.
    ...
    -   `media.audioOptions` **[Object][4]?** Options for configuring the call's audio.
        -   `media.audioOptions.deviceId` **[call.MediaConstraint][40]?** ID of the microphone to receive audio from.
    ...
    -   `media.videoOptions` **[Object][4]?** Options for configuring the call's video.
        -   `media.videoOptions.deviceId` **[call.MediaConstraint][40]?** ID of the camera to receive video from.
        -   `media.videoOptions.height` **[call.MediaConstraint][40]?** The height of the video.
        -   `media.videoOptions.width` **[call.MediaConstraint][40]?** The width of the video.
        -   `media.videoOptions.frameRate` **[call.MediaConstraint][40]?** The frame rate of the video.
    -   `media.screenOptions` **[Object][4]?** Options for configuring the call's screenShare.
        -   `media.screenOptions.height` **[call.MediaConstraint][40]?** The height of the screenShare.
        -   `media.screenOptions.width` **[call.MediaConstraint][40]?** The width of the screenShare.
        -   `media.screenOptions.frameRate` **[call.MediaConstraint][40]?** The frame rate of the screenShare.
-   `options` **[Object][4]?** 
    ...
    -   `options.customParameters` **[Array][7]&lt;[call.CustomParameter][32]>?** Custom SIP header parameters for the SIP backend
...
The custom parameters set on a call can be sent directly with the [call.sendCustomParameters][22] API.
...
-   `customParameters` **[Array][7]&lt;[call.CustomParameter][32]>** The custom parameters to set.
...
To change or remove the custom parameters on a call, use the [call.setCustomParameters][21] API.
...
Returns **[Array][7]&lt;[call.CallObject][44]>** Call objects.
...
-   `options` **[Object][4]?** Options for configuring the call's video.
    -   `options.deviceId` **[call.MediaConstraint][40]?** ID of the camera to receive video from.
    -   `options.height` **[call.MediaConstraint][40]?** The height of the video.
    -   `options.width` **[call.MediaConstraint][40]?** The width of the video.
    -   `options.frameRate` **[call.MediaConstraint][40]?** The frame rate of the video.
...
The SDK will emit a
   [call:statsReceived][48] event, after
...
-   `callId` **[string][5]** ID of the call being acted on.
-   `destination` **([call.SIP_URI][38] \| [call.TEL_URI][39])** The destination to forward the call to.
...
The progression of the operation will be tracked via the
   [call:operation][35] event. The remote
...
-   `callId` **[string][5]** ID of the call being acted on.
-   `destination` **([call.SIP_URI][38] \| [call.TEL_URI][39])** The destination to transfer the call to.
...
The progress of the operation will be tracked via the
   [call:operation][35] event. Both remote
...
The devices used for a call can be selected as part of the APIs for
   ...
   [call.make][19] and [call.answer][20] APIs, and speaker can be
...
Returns **[Array][7]** A list of call log records, ordered by latest first.

### getCache

Gets the cached call history data and returns stringified data.

Returns **[Array][7]** A list of call log records from the cache, ordered by latest first.
...
Returns **[Array][7]** A list of clickToCall records, ordered by earliest requestTime
...
Returns **[conversation.Conversation][53]** A Conversation object.
...
A message object can be obtained through the [Conversation.createMessage][54] API on an existing conversation.
...
-   `timestamp` **[number][6]** A Unix timestamp in seconds marking the time when the message was created by sender.
-   `parts` **[Array][7]&lt;conversation.Part>** An array of Part Objects.
-   `sender` **[string][5]** The primary contact address of the sender.
-   `destination` **[Array][7]&lt;[string][5]>** An array of primary contact addresses associated with various destinations to which the message is meant to be delivered.
...
-   `type` **[string][5]** The type of message that was sent. See [conversation.chatTypes][59] for valid types.
...
Returns **[Array][7]&lt;[Object][4]>** messages An array containing the conversation's messages.

Returns **[Function][9]** messages.markRead Marks the message as read.

Returns **[Function][9]** messages.forward Forward the message to another user.
...
Returns **[Array][7]** messages.parts The parts of the message.
...
-   `subscriber` **[Function][9]** A subscriber function to be triggered when the messages array of this conversation is updated.
...
Returns **[Function][9]** The unsubscribe function.
...
A [LogHandler][12] provided to the SDK (see
   [config.logs][61]) will need to handle LogEntry
   objects.
...
-   `timestamp` **[number][6]** When the log was created, based on UNIX epoch.
-   `method` **[string][5]** The log function that was used to create the log.
-   `level` **[string][5]** The level of severity the log.
-   `target` **[Object][4]** The subject that the log is about.
    -   `target.type` **[string][5]** The type of the target. This is also
           used as part of the name of the Logger.
    -   `target.id` **[string][5]?** A unique identifer for the target.
    -   `target.name` **[string][5]** A combination of the target type and ID. If no
           id was provided, this will be the same as the type.
-   `messages` **[Array][7]** The logged information, given to the Logger
...
A LogHandler can be provided to the SDK as part of its configuration (see
   [config.logs][61]). The SDK will then provide this
...
-   `LogEntry` **[Object][4]** The LogEntry to be logged.
...
Returns **[call.MediaObject][65]** A Media object.
...
-   `trackIds` **[Array][7]&lt;[string][5]>** List of Track IDs to be rendered.
...
-   `trackIds` **[Array][7]&lt;[string][5]>** List of Track IDs to stop being rendered.
-   `cssSelector` **[string][5]** A CSS selector string that uniquely
       identifies an element. Ensure that special characters are properly
       escaped.
...
-   `trackIds` **[Array][7]&lt;[string][5]>** List of Track IDs.
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][68]
in order for the SDK to process them.

**Parameters**

-   `params` **[Object][4]** 
    -   `params.services` **[Array][7]&lt;[string][5]>** Array of services for which we wish to receive notifications.
    -   `params.voipDeviceToken` **[string][5]** The voip device token used for voip push on iOS.
                                                 This token is required if registering for call service notifications on iOS.
    -   `params.standardDeviceToken` **[string][5]** The standardDevice token used for standard push on iOS .
                                                     This token is required when registering for non-call service notifications.
    -   `params.bundleId` **[string][5]** The bundleId to identify the application receiving the push notification.
    -   `params.clientCorrelator` **[string][5]** Unique identifier for a client device.
    -   `params.realm` **[string][5]** The realm used by the push registration service to identify and
...
Returns **[Promise][69]** When successful,  the information of the registration.
...
-   `params` **[Object][4]** 
    -   `params.services` **[Array][7]&lt;[string][5]>** Array of services to register for.
    -   `params.deviceToken` **[string][5]** The device token used for standard push on Android. This token is required
                                             when registering for all related services notifications.
    -   `params.bundleId` **[string][5]** The bundleId to identify the application receiving the push notification.
    -   `params.clientCorrelator` **[string][5]** Unique identifier for a client device.
    -   `params.realm` **[string][5]** The realm used by the push registration service to identify
...
-   `registrationInfo` **[string][5]** The data returned from the push registration

Returns **[Promise][69]** When successful, the promise will resolve with undefined.
                  Promise will reject with error object otherwise.
...
Presence information is persisted by the server. When the SDK is initialized,
   ...
   available either by using [presence.fetch][70] or by subscribing for
   updates about other Users, using [presence.subscribe][71].

Available presence information can be retrieved using [presence.get][72] or
   [presence.getAll][73].
...
An instance of this type can be obtained by invoking the [presence.get][72] function.
...
The status is set to [open][74] as soon as a user subscribes for the presence service.
...
-   `status` **[string][5]** The current status the user has set for themselves. For supported values see [presence.statuses][74].
-   `activity` **[string][5]** The current activity of the user.
         For supported values see [presence.activities][75].
...
See [presence.statuses][74] and [presence.activities][75] for valid
   values.

The SDK will emit a
   [presence:selfChange][76] event
   when the operation completes. The updated presence information is
   available and can be retrieved with [presence.getSelf][77].

Other users subscribed for this user's presence will receive a
   [presence:change][78] event.
...
-   `user` **([Array][7]&lt;[string][5]> | [string][5])** A User ID or an array of User IDs.

Returns **([Array][7]&lt;[Object][4]> | [Object][4])** List of user presence information.

### getAll

Retrieves the presence information for all available users.

Returns **[Array][7]&lt;[Object][4]>** List of user presence information.

### getSelf

Retrieves the presence information for the current user.

This information is set using the [presence.update][79] API.
...
Available presence information an be retrieved using the
   [presence.get][72] or [presence.getAll][73] APIs.
...
-   `users` **([Array][7]&lt;[string][5]> | [string][5])** A User ID or an array of User IDs.
...
-   `channel` **[proxy.Channel][80]** See the `Channel` module for information.
...
A set of [SdpHandlerFunction][81]s for manipulating SDP information.
...
-   `codecs` **[Array][7]&lt;[string][5]>** A list of codec names to remove from the SDP.
...
-   `subscribeUserList` **[Array][7]** The list of users to subcribe to.
-   `clientCorrelator` **[string][5]** 
-   `customParameters` **[Array][7]?** List of custom options provided as part of the subscription.
...
-   `userLists` **[Object][4]** 
    -   `userLists.subscribeUserList` **[Array][7]** The list of users to subcribe to.
    -   `userLists.unsubscribeUserList` **[Array][7]** The list of users to unsubscribe from. If all users are unsubscribed from, the event subscription is removed completly.
-   `customParameters` **[Array][7]?** List of custom options provided as part of the subscription.
...
The SDK will emit a [users:change][82]
...
Compared to [user.fetch][84] API, this API retrieves additional user related information.
...
See the [user.fetch][84] and [user.search][85] APIs for details about
...
Returns **[user.User][86]** The User object for the specified user.
...
Returns **[Array][7]&lt;[user.User][86]>** An array of all the User objects.
...
The SDK will emit a [directory:change][87]
   ...
   [user.get][83] and [user.getAll][88] APIs.
...
[21]: #callsetcustomparameters

[22]: #callsendcustomparameters

[23]: #calleventcallcustomparameters
...
[32]: #callcustomparameter

[33]: #callsip_uri

[34]: #calltel_uri

[35]: #calleventcalloperation

[36]: #calleventcallstart

[37]: #calleventcallreceive

[38]: #callsip_uri

[39]: #calltel_uri

[40]: #callmediaconstraint
...
[48]: #calleventcallstatsreceived

[49]: #calleventcalltrackreplaced
...
[53]: #conversationconversation

[54]: #conversationconversationcreatemessage

[55]: conversation.Message.addPart
...
[66]: #mediaeventmediamuted

[67]: #mediaeventmediaunmuted

[68]: api.notifications.process

[69]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[70]: #presencefetch

[71]: #presencesubscribe
...
[75]: #presenceactivities

[76]: #presenceeventpresenceselfchange

[77]: #presencegetself

[78]: #presenceeventpresencechange

[79]: #presenceupdate

[80]: #proxychannel

[81]: #callsdphandlerfunction

[82]: #usereventuserschange
